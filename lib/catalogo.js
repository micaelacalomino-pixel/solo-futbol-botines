...
export async function diagnosticoCatalogo() {
  ...
  if (!url) {
    return { error: "CATALOGO_CSV_URL no está definida" };
  }
  try {
...
export async function obtenerCatalogo() {
  ...
  const [encabezadoCrudo, ...resto] = filas;
  const encabezado = encabezadoCrudo.map((h) => h.trim());
  const idx = {
